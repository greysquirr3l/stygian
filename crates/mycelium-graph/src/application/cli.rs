//! Command-line interface for mycelium
//!
//! Provides the `mycelium` binary with subcommands for running, validating,
//! and visualising scraping pipelines.
//!
//! # Example
//!
//! ```text
//! mycelium run pipeline.toml
//! mycelium check pipeline.toml
//! mycelium list-services
//! mycelium list-providers
//! mycelium graph-viz pipeline.toml --format mermaid
//! ```
...
/// Mycelium — high-performance graph-based scraping engine
#[derive(Parser, Debug)]
#[command(
    name = "mycelium",
...
/// Available sub-commands for the mycelium CLI
...
mod tests {
    ...
    fn cli_parses_check_subcommand() {
        let cli = Cli::try_parse_from(["mycelium", "check", "pipeline.toml"]).unwrap();
        assert!(matches!(cli.command, Commands::Check { file } if file == "pipeline.toml"));
    }

    #[test]
    fn cli_parses_list_services() {
        let cli = Cli::try_parse_from(["mycelium", "list-services"]).unwrap();
        assert!(matches!(cli.command, Commands::ListServices));
    }

    #[test]
    fn cli_parses_list_providers() {
        let cli = Cli::try_parse_from(["mycelium", "list-providers"]).unwrap();
    ...
    fn cli_parses_graph_viz_dot() {
        let cli =
            Cli::try_parse_from(["mycelium", "graph-viz", "pipeline.toml", "--format", "dot"])
                .unwrap();
        assert!(matches!(
            cli.command,
            Commands::GraphViz {
    ...
    fn cli_parses_run_with_watch() {
        let cli = Cli::try_parse_from(["mycelium", "run", "pipeline.toml", "--watch"]).unwrap();
