...
//!
//! ```no_run
//! use mycelium_graph::application::extraction::{LlmExtractionService, ExtractionConfig};
...
impl LlmExtractionService {
    ...
    /// ```no_run
    /// use mycelium_graph::application::extraction::{LlmExtractionService, ExtractionConfig};
    /// use mycelium_graph::adapters::ai::ollama::OllamaProvider;
    /// use std::sync::Arc;
    ///
    /// let providers: Vec<Arc<dyn mycelium_graph::ports::AIProvider>> = vec![
    ...
    fn validate_output(output: &Value) -> Result<()> {
        if output.is_object() || output.is_array() {
            ...
            Err(MyceliumError::Provider(ProviderError::ApiError(format!(
...
impl ScrapingService for LlmExtractionService {
    ...
    async fn execute(&self, input: ServiceInput) -> Result<ServiceOutput> {
        if self.providers.is_empty() {
            return Err(MyceliumError::Provider(ProviderError::ApiError(
        ...
        let mut last_error: Option<MyceliumError> = None;
