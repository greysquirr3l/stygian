...
use crate::domain::error::{ConfigError, MyceliumError, Result};
...
///
/// ```rust
/// use mycelium_graph::application::graphql_plugin_registry::GraphQlPluginRegistry;
...
impl GraphQlPluginRegistry {
    ...
    /// # Errors
    ///
    /// Returns [`MyceliumError::Config`] wrapping [`ConfigError::MissingConfig`]
    ...
    pub fn get(&self, name: &str) -> Result<Arc<dyn GraphQlTargetPlugin>> {
        self.plugins.get(name).cloned().ok_or_else(|| {
            MyceliumError::Config(ConfigError::MissingConfig(format!(
...
mod tests {
    ...
    fn get_unknown_plugin_returns_error() {
        ...
        assert!(
            matches!(registry.get("unknown"), Err(MyceliumError::Config(_))),
