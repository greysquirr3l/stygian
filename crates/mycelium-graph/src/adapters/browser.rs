//! JavaScript rendering adapter using mycelium-browser
//!
//! Implements the `ScrapingService` port using a headless browser (via the
//! `mycelium-browser` crate) for pages that require JavaScript execution.
//!
//! Features:
//! - Full JS execution via Chrome DevTools Protocol
//! - Configurable wait strategies (DOM ready, network idle, selector)
//! - Stealth mode via mycelium-browser's anti-detection features
...
//!
//! ```no_run
//! use mycelium_graph::adapters::browser::{BrowserAdapter, BrowserAdapterConfig};
...
///
/// Wraps mycelium-browser's `BrowserPool` to implement the `ScrapingService` port.
...
impl BrowserAdapter {
    ...
    ///
    /// ```
    /// use mycelium_graph::adapters::browser::BrowserAdapter;
    ...
    /// Performs the browser navigation using mycelium-browser's `BrowserPool`.
    ///
    ...
    async fn navigate_with_browser(
        ...
        use mycelium_browser::page::WaitUntil;
    ...
    async fn navigate_with_browser(
        ...
        Err(MyceliumError::Service(ServiceError::Unavailable(format!(
            "mycelium-graph was compiled without the 'browser' feature; \
...
impl ScrapingService for BrowserAdapter {
    ...
    async fn execute(&self, input: ServiceInput) -> Result<ServiceOutput> {
        ...
        .await
        .map_err(|_| {
            MyceliumError::Service(ServiceError::Timeout(
...
mod tests {
    ...
    async fn test_execute_returns_service_output_or_unavailable() {
        ...
        match adapter.execute(input).await {
            ...
            Err(MyceliumError::Service(ServiceError::Unavailable(_))) => {
    ...
    async fn browser_adapter_respects_timeout() {
        ...
        match result {
            Err(MyceliumError::Service(ServiceError::Unavailable(msg))) => {
            ...
            Err(MyceliumError::Service(ServiceError::Timeout(_))) => {
