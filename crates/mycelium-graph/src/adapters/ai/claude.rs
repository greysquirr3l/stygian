...
//!
//! ```no_run
//! use mycelium_graph::adapters::ai::claude::{ClaudeProvider, ClaudeConfig};
...
impl ClaudeProvider {
    ...
    ///
    /// ```no_run
    /// use mycelium_graph::adapters::ai::claude::ClaudeProvider;
    ///
    ...
    fn parse_extract_response(response: &Value) -> Result<Value> {
        ...
        Err(MyceliumError::Provider(ProviderError::ApiError(
    ...
    fn map_http_error(status: u16, body: &str) -> MyceliumError {
        match status {
            401 => MyceliumError::Provider(ProviderError::InvalidCredentials),
            429 => MyceliumError::Provider(ProviderError::ApiError(format!(
                "Rate limited by Anthropic API: {body}"
            ))),
            400 => {
                if body.contains("token") {
                    MyceliumError::Provider(ProviderError::TokenLimitExceeded(body.to_string()))
                } else if body.contains("policy") {
                    MyceliumError::Provider(ProviderError::ContentPolicyViolation(body.to_string()))
                } else {
                    MyceliumError::Provider(ProviderError::ApiError(body.to_string()))
                }
            }
