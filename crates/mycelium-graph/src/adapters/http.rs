...
//!
//! ```no_run
//! use mycelium_graph::adapters::http::{HttpAdapter, HttpConfig};
...
impl HttpAdapter {
    ...
    ///
    /// ```no_run
    /// use mycelium_graph::adapters::http::HttpAdapter;
...
impl ScrapingService for HttpAdapter {
    async fn execute(&self, input: ServiceInput) -> Result<ServiceOutput> {
        let mut last_err: Option<MyceliumError> = None;
        ...
        for attempt in 0..=self.config.max_retries {
            ...
            match self.fetch(&input.url).await {
                ...
                Err(MyceliumError::Service(ServiceError::Unavailable(ref msg))) => {
                    ...
                    if retryable && attempt < self.config.max_retries {
                        last_err = Some(MyceliumError::Service(ServiceError::Unavailable(
                            msg.clone(),
                        )));
                        continue;
                    }
        ...
        Err(last_err.unwrap_or_else(|| {
            MyceliumError::Service(ServiceError::Unavailable("Max retries exceeded".into()))
        }))
    }
