...
For `StealthLevel::Advanced`, identity resolution is coherence-first by default:

1. Resolve one effective `FingerprintProfile` for the session (`BrowserConfig::fingerprint_profile`
    when provided, otherwise weighted random built-ins).
2. Derive navigator spoofing, fingerprint injection, WebGL noise profile,
    navigator-coherence script, peripheral stealth profile hints, and noise seed
    from that same effective profile.
3. Avoid contradictory defaults (for example hard-coded Windows/NVIDIA values)
    when a coherent profile identity is available.
