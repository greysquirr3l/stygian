...
//! use mycelium_browser::recorder::{SessionRecorder, RecorderConfig};
    ...
    fn build_har(&self) -> Har {
        ...
        Har {
            log: HarLog {
                ...
                creator: HarCreator {
                    name: "mycelium-browser".to_string(),
...
mod tests {
    ...
    fn export_har_writes_valid_json() -> std::result::Result<(), Box<dyn std::error::Error>> {
        ...
        let path = std::env::temp_dir().join("mycelium_test.har");
        r.export_har(&path)?;
    ...
    fn event_log_export_writes_ndjson() -> std::result::Result<(), Box<dyn std::error::Error>> {
        ...
        let path = std::env::temp_dir().join("mycelium_events.ndjson");
