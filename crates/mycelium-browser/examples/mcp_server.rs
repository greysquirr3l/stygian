...
//!   cargo run --example mcp_server -p mycelium-browser --features mcp
//! ```
//!
...
#[cfg(feature = "mcp")]
use mycelium_browser::mcp::{McpBrowserServer, is_mcp_enabled};
#[cfg(feature = "mcp")]
...
fn main() {
    ...
    eprintln!("Run with: cargo run --example mcp_server -p mycelium-browser --features mcp");
    std::process::exit(1);
}
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    tracing::info!("Starting mycelium-browser MCP server");
