//! Anti-detection validation test suite for mycelium-browser.
//!
...
//! cargo test -p mycelium-browser --test detection -- --ignored --test-threads=1
//!
//! # Run a single test
//! cargo test -p mycelium-browser --test detection stealth_webdriver_not_present -- --ignored
//! ```
//!
...
use mycelium_browser::{BrowserConfig, BrowserInstance, WaitUntil};
...
fn unique_user_data_dir() -> PathBuf {
    ...
    std::env::temp_dir().join(format!("mycelium-detect-{pid}-{n}"))
}
