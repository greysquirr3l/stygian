...
| `adaptive_rate_and_retry_budget` | `infer_requirements`: `blocked_ratio >= 0.10` from report totals | Elevated block ratio often indicates aggressive pacing/retry behavior | Primary: `@greysquirr3l` / Secondary: `@stygian-charon-on-call` | Needs explicit SLO for acceptable blocked ratio by target class (future: CHR-007) |
